<div x-data="{ isOpen: false }" class="relative">
    ...
    <div class="flex flex-col items-center cursor-pointer group w-fit" @mouseover="isOpen = true" @mouseout="isOpen = false">
        <!-- Icon -->
        <img src="{{ asset($iconDefault) }}" alt="Icon" class="h-12 w-12 transition-all group-hover:hidden" />
        <img src="{{ asset($iconHover) }}" alt="Icon" class="h-12 w-12 transition-all hidden group-hover:block" />

        <!-- Button text -->
        <span x-bind:class="{ 'underline': isOpen }" class="font-normal text-primary">{{ $text }}</span>

    </div>

    <!-- Hover Container  -->
    <div x-show="isOpen" class="fixed left-1/2 transform -translate-x-1/2 rounded z-10 mt-4" @mouseover="isOpen = true" @mouseout="isOpen = false" style="width: 90vw; max-width: 90vw; top:120px;" x-cloak>
